...
def voc2yolo(rootpath, xmlname, classes):
    ...
    dst_path = base_path.replace(ANNOTATIONS, 'labels_imglab')
    ...
    with open(xmlname, "r", encoding='UTF-8') as in_file:
        ...
        with open(txtfile, "w+", encoding='UTF-8') as out_file:
            ...
            for obj in root.iter('object'):
                ...
                # key =
                # xmin=float(key.find('xmin').text)/640
                # xmax=float(key.find('xmax').text)/640
                # ymin=float(key.find('ymin').text)/400
                # ymax=float(key.find('ymax').text)/400
                key_ponits=[]
                for keypoint in obj.find('keyPoints').findall('Point'):
